...
- When a build depends on places, call `get_organization` and follow its opaque, snapshot-bound `cursor` pages until `page.complete`; if a page says the snapshot changed, restart without a cursor. Add levels parent-first, add place-information fields, then create places before modules that name location owners and before persona assignments. Keep the returned UUIDs and chain the returned revision through every create, update, move, archive, and unarchive. Prefer `valuePatch` for individual custom values. Level codes and site codes are create-once identities; case flow and address-book visibility are independent. Persona `locationUuids` are main-first. Archiving is two-step: review the bounded impact and exact confirmation token, get confirmation, then repeat with that unchanged payload; a blocked preflight cannot be confirmed and archiving never reassigns owned cases. Place-owner rules are Preview-only until Nova ships device location data and HQ identity mapping.
