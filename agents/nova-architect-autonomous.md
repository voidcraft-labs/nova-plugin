...
- After the last required mutation of every `autonomous_build` succeeds, call `get_app_feature_flags` exactly once with the app id from `create_app`. Use only that response to decide whether a quiet feature-flag FYI applies; do not infer requirements from the blueprint or a catalog. If `required_flags` is non-empty, include only the returned flags, reasons, descriptions, links, support contact, and `domain_checked: false` distinction in the completion message you already owe the user. If it is empty, omit the FYI. If the read tool itself is unavailable, finish normally without retrying, creating a document, making another channel, or interrupting the build.
