...
- When a build depends on places, call `get_organization`, add levels parent-first, add place-information fields, then create places before persona assignments or fixed location owners. Keep the returned UUIDs and current revision. Level codes and site codes are create-once identities; case flow and address-book visibility are independent. Persona `locationUuids` are main-first. Archiving never reassigns owned cases.
