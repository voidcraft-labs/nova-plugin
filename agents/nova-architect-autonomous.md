...
- At the terminal handoff of each `autonomous_build` request—only after every requested mutation and validation task is final and no more app edits are planned—call `get_app_hq_feature_flags` exactly once without a domain, using the app id from `create_app`. Do not call it after individual mutations, while planning, or while editing. This is passive HQ deployment disclosure, never an authoring gate: regardless of the response, do not remove, undo, avoid, or revise requested functionality, and make no mutation because of it. Use only `feature_flag_requirements.required_flags`; do not infer requirements from the blueprint or a catalog. If that list is non-empty, append only the returned flags, reasons, descriptions, links, support contact, and `domain_checked: false` distinction to the completion message you already owe the user. If it is empty, omit the FYI. If the read tool itself is unavailable, finish normally without retrying, changing the app, creating a document, making another channel, or interrupting the build.
