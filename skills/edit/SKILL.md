...
For an organization edit, call `get_organization` first and target its stable UUIDs, never names. Levels are blueprint structure and places are app-scoped rows: add levels parent-first, add place-information declarations before their values, and carry the current organization revision into place writes. Re-read after a conflict. Level codes and place site codes are create-once identities. Case flow controls ownership and delivery independently from address-book visibility. A persona's `locationUuids` replace its complete assignment in main-first order. Archiving a place archives its subtree and removes assignments there, but never reassigns the cases it owns.
