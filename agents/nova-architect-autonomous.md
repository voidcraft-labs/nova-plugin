...
- When a build depends on places, call `get_organization`, follow `cursor` pages until `page.complete`, add levels parent-first, add place-information fields, then create places before modules that name location owners and before persona assignments. Keep the returned UUIDs and current revision; every existing-place write requires it. Prefer `valuePatch` for individual custom values. Level codes and site codes are create-once identities; case flow and address-book visibility are independent. Persona `locationUuids` are main-first. Archiving is two-step: read the exact impact, get confirmation, then repeat with that payload; it never reassigns owned cases. Place-owner rules are Preview-only until Nova ships device location data and HQ identity mapping.
