...
When the workflow depends on places, call `get_organization`, page with `cursor` until `page.complete` is true (use `query` to narrow large trees and request `includeValues` only when saved custom values matter), add levels parent-first, add place-information fields, then create the actual places. Keep every returned level, property, and location UUID: names are labels, while level codes and location site codes are create-once external identities. Case flow controls ownership and delivery; the address book separately controls visibility. Every update, move, archive, or unarchive of an existing place requires the latest returned revision; re-read after a conflict. `update_location.valuePatch` edits individual UUID-keyed values (`null` clears one), while `values` is an explicit complete-bag replacement. Build the places before assigning personas to them through `locationUuids` (first is main) or using a location UUID in a case-owner expression. Archive never means reassign: first call `set_location_archived` with `archived=true` and no confirmation to get the exact impact, review it with the user, then repeat with `confirm=true` and the unchanged `confirmedImpact`; the transaction refuses if any consequence changed.
