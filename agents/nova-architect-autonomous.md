...
- When a build requests automations, load the complete automation tool family and add each rule only after its case types, forms, worker information, and places exist. Keep stable UUIDs for the rule and every nested item and use only the loaded schema's closed vocabulary. UCR/custom criteria and HQ server-modified behavior are setup-only. Nova may count current real open cases and name omissions, but it never updates cases, sends messages, advances schedules, or installs rules in CommCare HQ. Preserve and report the returned manual setup guidance; never imply that app upload configures the automation.
